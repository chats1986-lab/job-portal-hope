...
public class JobCategoryMapper {
    public static JobCategoryResponse toJobCategoryResponse(JobCategory category, boolean includeChildren) {
        ...
        if(includeChildren && category.getSubCategories() != null && category.getSubCategories().size() > 0) {
            category.getSubCategories().stream().map(sub -> toJobCategoryResponse(sub, false))
